...
async function modalAdd() {
    const promise = new Promise(async (resolve, reject) => {
        let modalName = this.dataset.modal,
            modalItem = await fetch(`https://nautz.ru/new-constructor/modal/en/oneof/test/_${modalName}.html`);
        ...
        if (modalItem.ok) {
            ...
            if (this.dataset.modal === "form") {
                if (typeof formFlag === "undefined") {
                    await $.getScript("https://nautz.ru/new-constructor/js/en/oneof/test/_form.js");
                }
                resolve(modalForm())
            }
        }
...
function createTempOrder(){
    ...
    $.ajax({
        url: 'https://nautz.loc/api/bitrix/create_temp_order_nc', // тут подразумевается что пока скрипт будет тестироваться на nautz, потом на тильде надо будет указвать полный путь
        type: 'post',
        dataType: 'json',
        data: for_api,
    })
