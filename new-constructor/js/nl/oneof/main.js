...
async function modalAdd() {
    const promise = new Promise(async (resolve, reject) => {
        let modalName = this.dataset.modal,
            modalItem = await fetch(`https://nautz.ru/new-constructor/modal/nl/oneof/_${modalName}.html`);
        modal.classList.add("active");

        if (this.dataset.modal == "form") {
            modal.classList.add("full");
            calc_total("sweatshirt met capuchon", finalPrice)
            // закончил квиз
        ...
        if (modalItem.ok) {
            ...
            if (this.dataset.modal === "form") {
                if (typeof formFlag === "undefined") {
                    await $.getScript("https://nautz.ru/new-constructor/js/nl/oneof/_form.js");
                }
                resolve(modalForm())
            }
        }
